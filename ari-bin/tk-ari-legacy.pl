...
# Logger #
##########
my $log_file = "/opt/telekinesis/logs/legacy-ari.$radio.log";
...
# cli interface
my $socket = '/opt/telekinesis/run/ari.sock';

my $ari_pass_file = "/opt/telekinesis/run/ari.$radio.pass";
...
###############################################
print "tk-ari-legacyd.pl version <$VERSION>\n";
...
sub ari_speech {
   my $chan_id = $_[0];
   my $prompt = $_[1];
   ari_play($chan_id, "sound:tk/" . $client->{'tts_voice'} . "/" . $prompt);
}
...
sub start_baresip_channel {
   ...
   my $file = "/opt/telekinesis/logs/baresip-$chan_name.log";
   open my $fh, '>', $file;
   defined(my $pid = fork) or die "fork: $!";
   if (!$pid) {
      open STDOUT, '>&', $fh;
      exec("baresip", ("baresip", "-f", "/opt/telekinesis/tmp/baresip-$radio", "-e", "$dialstring", "-d"));
   }
   waitpid $pid, 0;
   my $retcode = $?;
